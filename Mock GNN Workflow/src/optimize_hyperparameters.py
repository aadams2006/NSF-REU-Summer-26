...
This script runs a small, reproducible search over curated configurations,
saves every trial under a uniquely labeled search directory, and promotes
the strongest configuration into the standard training results directory.
"""
...
def read_key_value_file(file_path):
    """
    Read a simple key=value text file.
    """
...
def read_metrics_summary(metrics_path):
    """
    Read the metrics summary CSV into a dictionary.
    """
...
def create_search_context():
    """
    Create a uniquely labeled hyperparameter-search directory.
    """
...
def write_search_metadata(search_context, baseline_info, validation_trial, promotion_trial,
                          promotion_reason, promoted_run):
    """
    Save top-level metadata for the search.
    """
...
def write_trial_results(search_context, trial_rows):
    """
    Save a sortable CSV of all trial results.
    """
...
def write_best_result_summary(search_context, baseline_info, validation_trial, promotion_trial,
                              promotion_reason, promoted_run):
    """
    Save a compact comparison between the baseline and selected configuration.
    """
    ...
    with open(summary_path, 'w', newline='') as csvfile:
        ...
        if baseline_metrics:
            baseline_test_mse = float(baseline_metrics['test_mse'])
            baseline_test_mae = float(baseline_metrics['test_mae'])
            baseline_test_r2 = float(baseline_metrics['test_r2'])
            writer.writerow(['delta_test_mse', promoted_metrics['mse'] - baseline_test_mse])
            writer.writerow(['delta_test_mae', promoted_metrics['mae'] - baseline_test_mae])
            writer.writerow(['delta_test_r2', promoted_metrics['r2'] - baseline_test_r2])


def get_baseline_info():
    """
    Read the latest standard training run before optimization begins.
    """
...
def run_trial(trial_index, config, data_list, labels, device, search_context):
    """
    Train a single trial and save its metrics under the search directory.
    """
...
def promote_best_config(best_config, data_list, labels, device):
    """
    Re-run the best configuration through the standard training output flow.
    """
...
def main():
    ...
    if baseline_info is not None:
        print(f"Baseline run: {baseline_info.get('run_label', 'unknown')}")
        print(f"Baseline test MSE: {baseline_info.get('metrics', {}).get('test_mse', 'n/a')}")
    ...
    for trial_index, overrides in enumerate(TRIAL_OVERRIDES, start=1):
        ...
        print(
            f"\nTrial {trial_index:02d}/{len(TRIAL_OVERRIDES)}: "
            f"hidden={config['hidden_dim']}, layers={config['num_layers']}, "
            f"dropout={config['dropout']}, batch={config['batch_size']}, "
            f"lr={config['learning_rate']}, wd={config['weight_decay']}, seed={config['seed']}"
        )
        ...
        print(
            f"  best_val_loss={trial_row['best_val_loss']:.6f}, "
            f"test_mse={trial_row['test_mse']:.6f}, "
            f"test_mae={trial_row['test_mae']:.6f}, "
            f"test_r2={trial_row['test_r2']:.6f}"
        )
    ...
    write_best_result_summary(
        search_context, baseline_info, best_validation_row,
        promotion_trial, promotion_reason, promoted_run
    )
    ...
    print(f"Best validation trial: {best_validation_row['trial_label']}")
    print(f"Best validation loss: {best_validation_row['best_val_loss']:.6f}")
    print(f"Promotion trial: {promotion_trial['trial_label']} ({promotion_reason})")
    print(f"Promoted run: {promoted_run['run_context']['run_label']}")
    print(f"Promoted test MSE: {promoted_run['history']['test_metrics']['mse']:.6f}")
